...
def get_consulta_producto(codigo_barras):
    """
    Consulta detallada de un producto específico por código de barras.
    Retorna toda la información relevante: existencias, ventas, velocidad, etc.
    """
    import sqlite3
    import pandas as pd
    from datetime import datetime, timedelta
    ...
    with get_connection() as conn:
        ...
        LIMIT 1
        """
        df_info = pd.read_sql(query_info, conn, params=(codigo_barras,))
        
        if df_info.empty:
            # También buscar en inventario_bodega_raw
            query_info_bodega = """
            SELECT DISTINCT
                c_barra,
            ...
            LIMIT 1
            """
            df_info = pd.read_sql(query_info_bodega, conn, params=(codigo_barras,))
            if df_info.empty:
                return {
                    "encontrado": False,
        ...
        # ===== 2. COMBINAR EXISTENCIAS DE AMBAS TABLAS =====
        # Existencias en tiendas (ventas_saldos_raw)
        query_existencias_tiendas = """
        SELECT 
            COALESCE(ct.clean_name, s.d_almacen) AS tienda,
            ct.region,
            s.saldo_disponible AS stock_actual
        ...
        LEFT JOIN config_tiendas ct ON s.d_almacen = ct.raw_name
        WHERE s.c_barra = ?
        ORDER BY ct.region, tienda
        """
        df_existencias_tiendas = pd.read_sql(query_existencias_tiendas, conn, params=(codigo_barras,))
        
        # Existencias en bodega (inventario_bodega_raw)
        query_existencias_bodega = """
        SELECT 
        ...
        WHERE c_barra = ?
        """
        df_existencias_bodega = pd.read_sql(query_existencias_bodega, conn, params=(codigo_barras,))
        
        # Combinar ambos DataFrames
        df_existencias = pd.concat([df_existencias_tiendas, df_existencias_bodega], ignore_index=True)
        
        # Calcular totales
        stock_total = int(df_existencias['stock_actual'].sum())
        stock_bodega = int(df_existencias[
            df_existencias['tienda'].str.contains('BODEGA', case=False, na=False)
        ]['stock_actual'].sum())
        ...
        AND DATE(substr(f_sistema,7,4)||'-'||substr(f_sistema,4,2)||'-'||substr(f_sistema,1,2))
            >= DATE('now', '-30 days')
        """
        ventas_30 = pd.read_sql(query_ventas_30, conn, params=(codigo_barras,))['ventas'].iloc[0] or 0
        ...
        AND DATE(substr(f_sistema,7,4)||'-'||substr(f_sistema,4,2)||'-'||substr(f_sistema,1,2))
            >= DATE('now', '-60 days')
        """
        ventas_60 = pd.read_sql(query_ventas_60, conn, params=(codigo_barras,))['ventas'].iloc[0] or 0
        ...
        AND DATE(substr(f_sistema,7,4)||'-'||substr(f_sistema,4,2)||'-'||substr(f_sistema,1,2))
            >= DATE('now', '-90 days')
        """
        ventas_90 = pd.read_sql(query_ventas_90, conn, params=(codigo_barras,))['ventas'].iloc[0] or 0
        ...
        # ===== 5. ÚLTIMA FECHA DE VENTA =====
        query_ultima_venta = """
        SELECT MAX(DATE(substr(f_sistema,7,4)||'-'||substr(f_sistema,4,2)||'-'||substr(f_sistema,1,2))) as ultima_venta
        FROM ventas_historico_raw
        WHERE c_barra = ?
        """
        ultima_venta = pd.read_sql(query_ultima_venta, conn, params=(codigo_barras,))['ultima_venta'].iloc[0]
        
        # ===== 6. VENTAS POR TIENDA (últimos 30 días) - SOLO TIENDAS, NO BODEGA =====
        query_ventas_tienda = """
        SELECT 
            COALESCE(ct.clean_name, h.d_almacen) AS tienda,
        ...
        GROUP BY tienda
        """
        df_ventas_tienda = pd.read_sql(query_ventas_tienda, conn, params=(codigo_barras,))
        
        # Unir existencias con ventas (solo para tiendas, no bodega)
        df_distribución = df_existencias_tiendas.merge(
            df_ventas_tienda,
            on='tienda',
            how='left'
        ).fillna({'ventas_30d': 0})
        
        df_distribución['ventas_30d'] = df_distribución['ventas_30d'].astype(int)
        
        # Agregar bodega al DataFrame de distribución si existe
        if not df_existencias_bodega.empty:
            bodega_row = df_existencias_bodega.iloc[0].to_dict()
            bodega_row['ventas_30d'] = 0  # Bodega no tiene ventas
            df_distribución = pd.concat([
                df_distribución, 
                pd.DataFrame([bodega_row])
            ], ignore_index=True)
        
        # ===== 7. TIENDAS SIN EL PRODUCTO =====
        query_todas_tiendas = """
        SELECT DISTINCT clean_name as tienda
        FROM config_tiendas
        WHERE clean_name NOT LIKE '%BODEGA%'
        """
        todas_tiendas = pd.read_sql(query_todas_tiendas, conn)['tienda'].tolist()
        tiendas_con_producto = df_existencias_tiendas[
            ~df_existencias_tiendas['tienda'].str.contains('BODEGA', case=False, na=False)
        ]['tienda'].tolist()
        tiendas_sin_producto = [t for t in todas_tiendas if t not in tiendas_con_producto]
        
        # ===== 8. HISTORIAL DE MOVIMIENTOS (últimos 30 días) =====
        query_historial = """
        SELECT 
        ...
        LIMIT 50
        """
        df_historial = pd.read_sql(query_historial, conn, params=(codigo_barras,))
        
        # ===== 9. VENTAS DIARIAS PARA GRÁFICO (últimos 30 días) =====
        query_grafico = """
        SELECT 
            DATE(substr(f_sistema,7,4)||'-'||substr(f_sistema,4,2)||'-'||substr(f_sistema,1,2)) as fecha,
        ...
        ORDER BY fecha
        """
        df_grafico = pd.read_sql(query_grafico, conn, params=(codigo_barras,))
        
        # ===== 10. SUGERENCIA DE REABASTECIMIENTO =====
        if velocidad_dia == 0:
            sugerencia = "❌ Sin movimiento - Considerar redistribución"
            estado = "sin_movimiento"
        elif dias_agotar < 15:
            sugerencia = f"🔴 URGENTE - Reabastecer (quedan {dias_agotar} días)"
            estado = "critico"
        elif dias_agotar < 30:
            sugerencia = f"🟡 Programar reabastecimiento ({dias_agotar} días de stock)"
            estado = "alerta"
        else:
            sugerencia = f"✅ Stock óptimo ({dias_agotar} días de stock)"
            estado = "optimo"
        
        # ===== 11. VALOR DEL INVENTARIO =====
        # Intentar obtener costo unitario desde tiendas o bodega
        try:
            query_costo = """
            SELECT COALESCE(costo_unitario, precio_venta * 0.6, 0) as costo
            FROM ventas_saldos_raw
            WHERE c_barra = ?
            LIMIT 1
            """
            costo_unit = pd.read_sql(query_costo, conn, params=(codigo_barras,))['costo'].iloc[0]
            if pd.isna(costo_unit) or costo_unit == 0:
                # Intentar desde bodega si no hay en tiendas
                query_costo_bodega = """
                SELECT 1.0 as costo  -- Valor por defecto si no hay costo
                FROM inventario_bodega_raw
                WHERE c_barra = ?
                LIMIT 1
                """
                df_costo = pd.read_sql(query_costo_bodega, conn, params=(codigo_barras,))
                if not df_costo.empty:
                    costo_unit = 1.0  # Valor por defecto razonable
                else:
                    costo_unit = 0
            valor_inventario = round(stock_total * costo_unit, 2) if costo_unit else None
        except:
        ...
        return {
            ...
            "ventas": {
                "ultimos_30_dias": int(ventas_30),
                "ultimos_60_dias": int(ventas_60),
                "ultimos_90_dias": int(ventas_90),
                "velocidad_dia": velocidad_dia,
                "dias_para_agotar": dias_agotar,
                "ultima_fecha_venta": ultima_venta
            },
            "distribucion": df_distribución.to_dict(orient='records'),
